"""
Phase 1 v4: Select validation cohort with TREND DATA — CORRECTED ORDERING.
Trend arrays are now in NEWEST-FIRST order, matching the system's expected convention
(preprocessor + guardrail both interpret array[0] as the latest reading).

Bug fix from v2: previously emitted oldest-first, which caused the system to
treat 5-hour-old values as current snapshots.
"""
...
def main():
    print("=" * 60)
    print("Phase 1 v4: Select Cohort with TREND DATA (NEWEST-FIRST, bug fix)")
    print("=" * 60)
    ...
    with open(manifest_path, "w") as f:
        json.dump({
            ...
            "version": "v4 - 6h trend data, NEWEST-FIRST order (bug fix from v2)",
