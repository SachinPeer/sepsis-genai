"""
Phase 1 v5: Select validation cohort with 4-HOUR-PRIOR SNAPSHOT (6h trend unchanged).

Hypothesis:
  PhysioNet's SepsisLabel fires at t_clinical_sepsis - 6h. Our v4 cohort took
  the snapshot exactly at that label-onset hour, so the system was asked to
  predict 6h ahead. Without real nurse notes, the LLM relied on vitals alone
  and often missed early sepsis cases that still looked "stable."

  v5 simply shifts the SNAPSHOT 2 hours forward — so we look at the patient
  4h before clinical sepsis instead of 6h. The trend window stays at 6 hours.
  The system effectively sees 2 hours of additional disease progression in
  the snapshot vitals, which should help catch deteriorating patients.

Compared to v4:
  - Sepsis snapshot:  onset_idx + 2  (was: onset_idx). Now 4h-before clinical.
  - Trend window:     6 hours (UNCHANGED from v4).
  - Non-sepsis snapshot: still mid-record (UNCHANGED from v4).
  - Trend arrays:     still NEWEST-FIRST (preserves v4 bug fix).
"""
...
COHORT_DIR = Path(__file__).parent / "selected_cohort_v5"
COHORT_DIR.mkdir(exist_ok=True)

TARGET_SEPSIS = 140
TARGET_NON_SEPSIS = 200
TREND_HOURS = 6  # SAME as v4 — only the snapshot moves
SEPSIS_SNAPSHOT_OFFSET = 2  # snapshot at onset_idx + 2 = 4h before clinical sepsis
...
def extract_trend_vitals(rows, target_idx):
    """Extract 6 hours of trend data, NEWEST-FIRST order (same as v4)."""
    ...
    for col_name, api_name in vital_cols.items():
        ...
        if trend_points:
            # Newest-first: reverse the chronological list
            vitals[api_name] = list(reversed(trend_points))
...
def build_rich_notes(rows, target_idx):
    """Build detailed nurse notes from 6 hours of trend data (same as v4)."""
    start_idx = max(0, target_idx - TREND_HOURS + 1)
    trend_rows = rows[start_idx:target_idx + 1]
    notes_parts = []
    ...
    if not notes_parts:
        return "Patient in ICU, routine monitoring. Vitals within expected range for ICU baseline. No acute changes noted."
...
def main():
    print("=" * 60)
    print("Phase 1 v5: Cohort with 4h-PRIOR SNAPSHOT (6h trend unchanged)")
    print("(snapshot 2 hours closer to clinical sepsis vs v4)")
    print("=" * 60)
    ...
    # Need TREND_HOURS history before snapshot AND SEPSIS_SNAPSHOT_OFFSET hours after onset
    min_required_for_sepsis = TREND_HOURS  # at least 8 hours before snapshot
    ...
    for i, filename in enumerate(psv_files):
        ...
        if onset is not None:
            target_idx = onset + SEPSIS_SNAPSHOT_OFFSET
            if target_idx < len(rows) and target_idx >= TREND_HOURS - 1:
                sepsis_candidates.append({
                    "filename": filename,
                    "rows": rows,
                    "onset_idx": onset,
    ...
    print(f"\nScan complete: {len(sepsis_candidates)} sepsis-eligible, {len(non_sepsis_candidates)} non-sepsis")
    ...
    for p in selected_sepsis:
        ...
        vitals = extract_trend_vitals(p["rows"], target_idx)
        notes = build_rich_notes(p["rows"], target_idx)

        api_input = {"patient_id": patient_id, "vitals": vitals, "notes": notes}
        ground_truth = {
            "patient_id": patient_id, "actual_sepsis": True,
            "onset_hour": p["onset_idx"],
            "snapshot_hour": target_idx,
            "snapshot_offset_from_onset": SEPSIS_SNAPSHOT_OFFSET,
    ...
    for p in selected_non_sepsis:
        patient_id = p["filename"].replace(".psv", "")
        # Use mid-record snapshot, ensure enough history
        target_idx = max(TREND_HOURS - 1, len(p["rows"]) // 2)
        if target_idx >= len(p["rows"]):
            target_idx = len(p["rows"]) - 1
        vitals = extract_trend_vitals(p["rows"], target_idx)
        notes = build_rich_notes(p["rows"], target_idx)
    ...
    with open(manifest_path, "w") as f:
        json.dump({
            ...
            "version": "v5 - 4h-prior snapshot (onset+2), 6h trend window (same as v4), NEWEST-FIRST",
            "trend_hours": TREND_HOURS,
            "sepsis_snapshot_offset": SEPSIS_SNAPSHOT_OFFSET,
            "selection_seed": 42,
            "patients": cohort_manifest
        }, f, indent=2)

    print(f"Cohort saved to: {COHORT_DIR}")
    print(f"Total: {len(cohort_manifest)} patients")
    print(f"Trend window: {TREND_HOURS} hours, NEWEST-FIRST")
    print(f"Sepsis snapshot: {SEPSIS_SNAPSHOT_OFFSET}h after onset (= 4h before clinical sepsis)")

    sample = COHORT_DIR / f"{selected_sepsis[0]['filename'].replace('.psv', '')}.json"
    with open(sample, "r") as f:
        s = json.load(f)
    print(f"\nSample sepsis patient ({s['ground_truth']['patient_id']}):")
    print(f"  Snapshot at hour {s['ground_truth']['snapshot_hour']} (onset was at {s['ground_truth']['onset_hour']})")
    print(f"  Notes: {s['api_input']['notes'][:200]}...")
